...
- `scripts/`: operational tooling (for example, OpenClaw proxy injection helpers).
...
- `docker compose up -d --build`: start default base stack (AegisGate only).
- `docker compose -f docker-compose.yml -f docker-compose.cliproxy.yml up -d --build`: start optional CLIProxyAPI + Caddy optimized stack.
