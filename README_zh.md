...
- **OpenAI 兼容 API** — 直接替换 `/v1/chat/completions`、`/v1/responses`、`/v1/messages` 及通用代理；兼容任何 OpenAI 兼容服务商
- **Anthropic ↔ OpenAI 协议转换** — Token 级 `compat` 模式自动将 Anthropic `/v1/messages` 请求转为 OpenAI `/v1/chat/completions` 格式，Claude Code / Anthropic SDK 无需改代码即可对接 OpenAI 兼容上游（GPT-5.4 等）
...
- OpenAI 兼容接口：
  ...
  - `POST /v1/responses`
  - `POST /v1/messages` — Anthropic Messages 专用端点（完整安全管道）；支持原样透传到 Anthropic 兼容上游，或通过 token `compat` 模式自动转换为 OpenAI Chat Completions 格式
...
当 token 配置了 `"compat": "openai_chat"` 时，网关自动将 Anthropic `/v1/messages` 请求转为 OpenAI `/v1/chat/completions` 格式，并将响应转回。Claude Code 和 Anthropic SDK 无需改代码即可对接 OpenAI 兼容上游。
...
| URL | 行为 |
|-----|------|
| `/v1/__gw__/t/claude-to-gpt/8317/messages` | Messages → Chat → `:8317` → 响应转回 Messages |
