...
def confirmation_template(confirm_id: str, reason: str, summary: str) -> str:
    return (
        ...
        f"可疑简述：{summary}\n\n"
        "⚠️ 安全确认（高风险操作）\n"
        "我检测到你的上一条请求可能包含高风险指令（例如：删除文件/文件夹、执行命令、读取本地敏感信息、修改关键配置等）。"
        "为避免误操作或被提示注入利用，本次请求已被网关暂停执行。\n\n"
        "请你仅回复以下之一：\n\n"
        ...
        "I detected that your previous request may contain a high-risk instruction (e.g., deleting files/folders, executing commands, "
        "accessing local sensitive data, or changing critical settings). To prevent accidents or prompt-injection abuse, the gateway has paused this request.\n\n"
