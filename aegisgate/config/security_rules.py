...
_DEFAULT_RULES: dict[str, Any] = {
    ...
    "anomaly_detector": {
        ...
        "command_patterns": [
            {"id": "rm_rf", "regex": r"rm\s+-rf"},
            {"id": "curl_pipe_sh", "regex": r"curl\s+[^|]+\|\s*(sh|bash)"},
            {"id": "powershell_enc", "regex": r"powershell(\.exe)?\s+-enc"},
            {"id": "certutil_decode", "regex": r"certutil\s+-decode"},
            {"id": "reg_add", "regex": r"reg\s+add"},
            {"id": "cat_ssh", "regex": r"cat\s+~/.ssh"},
            {"id": "zh_priv_escalation", "regex": r"(提权|越权|读取本地|导出敏感|删除日志)"},
        ],
        "points": {
    ...
    "request_sanitizer": {
        ...
        "command_patterns": [
            {"id": "shell_en", "regex": r"(rm\s+-rf|curl\s+[^|]+\|\s*(sh|bash)|powershell(\.exe)?\s+-enc|cmd\.exe)"},
            {"id": "shell_zh", "regex": r"(执行命令|运行脚本|终端执行|读取本地文件)"},
        ],
    ...
    "sanitizer": {
        ...
        "force_block_command_patterns": [
            {"id": "docker_compose_down", "regex": r"\b(?:docker\s+compose|docker-compose)\s+down\b"},
            {"id": "docker_system_prune", "regex": r"\bdocker\s+system\s+prune(?:\s+-[a-z]+)*\b"},
            {"id": "docker_run_privileged", "regex": r"\bdocker\s+run\b[^\n]*\s--privileged\b"},
            {"id": "docker_mount_root_fs", "regex": r"\bdocker\s+run\b[^\n]*(?:-v|--volume)\s*/:\s*/[A-Za-z0-9._/-]+\b"},
            {
                "id": "docker_mount_docker_sock",
                "regex": r"\bdocker\s+run\b[^\n]*(?:-v|--volume)\s*/var/run/docker\.sock\s*:\s*/var/run/docker\.sock\b",
            },
            {"id": "docker_net_host", "regex": r"\bdocker\s+run\b[^\n]*--net(?:work)?=host\b"},
            {"id": "docker_cap_add_all", "regex": r"\bdocker\s+run\b[^\n]*--cap-add=ALL\b"},
            {
                "id": "docker_unconfined_security_opt",
                "regex": r"\bdocker\s+run\b[^\n]*--security-opt\s+(?:seccomp|apparmor)=unconfined\b",
            },
            {
                "id": "ssh_authorized_keys_write",
                "regex": r"(?:>>|>|(?:\|\s*tee\b)|\bAdd-Content\b|\bSet-Content\b|\bOut-File\b).*(?:~?/\.ssh/authorized_keys|/root/\.ssh/authorized_keys|C:\\ProgramData\\ssh\\administrators_authorized_keys)",
            },
            {
                "id": "ssh_private_key_write",
                "regex": r"(?:>>|>|(?:\|\s*tee\b)|\bAdd-Content\b|\bSet-Content\b|\bOut-File\b).*(?:~?/\.ssh/(?:id_rsa|id_ed25519)|/root/\.ssh/(?:id_rsa|id_ed25519)|\S+\.pem\b)",
            },
            {
                "id": "sshd_config_write",
                "regex": r"(?:>>|>|(?:\|\s*tee\b)|\bsed\s+-i\b|\bperl\s+-pi\b).*(?:/etc/ssh/sshd_config(?:\.d/\S+)?)",
            },
            {
                "id": "sshd_weak_setting",
                "regex": r"\b(?:PermitRootLogin\s+yes|PasswordAuthentication\s+yes|PubkeyAuthentication\s+no|PermitEmptyPasswords\s+yes)\b",
            },
            {
                "id": "ssh_sensitive_file_exfil",
                "regex": r"\b(?:cat|type|more|less|Get-Content|scp|rsync|curl|wget|Invoke-WebRequest|Invoke-RestMethod)\b.*(?:/etc/ssh/\S*key\S*|~?/\.ssh/(?:authorized_keys|known_hosts|id_rsa|id_ed25519))",
            },
            {
                "id": "ssh_archive_exfil",
                "regex": r"\b(?:tar|zip|7z)\b.*(?:~?/\.ssh\b|/etc/ssh\b).*(?:\||&&|;).*(?:curl|wget|nc|scp|rsync|Invoke-WebRequest|Invoke-RestMethod)\b",
            },
            {"id": "iptables_flush", "regex": r"\biptables\s+(?:-F|--flush)\b"},
            {"id": "nft_flush_ruleset", "regex": r"\bnft\s+flush\s+ruleset\b"},
            {"id": "iptables_policy_accept", "regex": r"\biptables\s+-P\s+(?:INPUT|FORWARD|OUTPUT)\s+ACCEPT\b"},
            {"id": "ufw_disable", "regex": r"\bufw\s+disable\b"},
            {"id": "pfctl_disable", "regex": r"\bpfctl\s+-d\b"},
            {"id": "netsh_firewall_off", "regex": r"\bnetsh\s+advfirewall\s+set\s+allprofiles\s+state\s+off\b"},
            {"id": "powershell_firewall_off", "regex": r"\bSet-NetFirewallProfile\b[^\n]*\s-Enabled\s+False\b"},
            {
                "id": "windows_inbound_remote_open",
                "regex": r"\b(?:New-NetFirewallRule|netsh\s+advfirewall\s+firewall\s+add\s+rule)\b[^\n]*(?:\bdir(?:ection)?\s*=?\s*in\b|\b-Inbound\b)[^\n]*(?:\b22\b|\b3389\b|\b2375\b|\b5985\b|\b5986\b)",
            },
        ],
        "encoded_payload_patterns": [
            {"id": "base64_long", "regex": r"[A-Za-z0-9+/]{200,}={0,2}"},
