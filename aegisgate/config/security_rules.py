...
_DEFAULT_RULES: dict[str, Any] = {
    ...
    "sanitizer": {
        ...
        "force_block_command_patterns": [
            ...
            {"id": "docker_system_prune", "regex": r"(?:^|\s)(?:/)?docker\s+system\s+prune(?:\s+-[a-z]+)*\b"},
            ...
            {"id": "docker_run_privileged", "regex": r"(?:^|\s)(?:/)?docker\s+run\b[^\n]*\s--privileged\b"},
            {"id": "docker_mount_root_fs", "regex": r"(?:^|\s)(?:/)?docker\s+run\b[^\n]*(?:-v|--volume)\s*/:\s*/[A-Za-z0-9._/-]+\b"},
            {
                "id": "docker_mount_docker_sock",
                "regex": r"(?:^|\s)(?:/)?docker\s+run\b[^\n]*(?:-v|--volume)\s*/var/run/docker\.sock\s*:\s*/var/run/docker\.sock\b",
            },
            {"id": "docker_net_host", "regex": r"(?:^|\s)(?:/)?docker\s+run\b[^\n]*--net(?:work)?=host\b"},
            {"id": "docker_cap_add_all", "regex": r"(?:^|\s)(?:/)?docker\s+run\b[^\n]*--cap-add=ALL\b"},
            {
