...
async def test_gw_add_rejects_upstream_pair_conflict(monkeypatch):
    monkeypatch.setattr(
        gateway,
        "gw_tokens_get",
        lambda token: {"upstream_base": "https://old.example.com/v1", "whitelist_key": []}
        if token == "tok123"
        else None,
    )
...
async def test_gw_add_rejects_gateway_key_mismatch(monkeypatch):
    monkeypatch.setattr(
        gateway,
        "gw_tokens_get",
        lambda token: {"upstream_base": "https://gmn.chuangzuoli.com/v1", "whitelist_key": []}
        if token == "tok123"
        else None,
    )
...
async def test_gw_remove_returns_404_when_update_fails(monkeypatch):
    monkeypatch.setattr(
        gateway,
        "gw_tokens_get",
        lambda token: {"upstream_base": "https://gmn.chuangzuoli.com/v1", "whitelist_key": ["bn_key"]}
        if token == "tok123"
        else None,
    )
    monkeypatch.setattr(gateway, "gw_tokens_update", lambda *_args, **_kwargs: False)
