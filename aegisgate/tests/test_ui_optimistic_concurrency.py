...
class TestSuccessfulWriteHandsBackAUsableValidator:
    """A save must leave the client able to save again.

    The console tracks one ETag per logical resource and only ever refreshed it
    from reads. Panels that reload after a write (rules, exact values) recovered
    by accident; the compose editor and the action map do not reload, so their
    second consecutive save sent the pre-write validator and was rejected as a
    conflict that never happened — and the conflict handler then reloaded the
    view, discarding the edit the user had just made.

    The fix is for a *successful* write to hand back the post-write validator,
    which these tests pin: chaining If-Match from each response must let a client
    write repeatedly without ever re-reading.
    """

    def test_compose_can_be_saved_twice_in_a_row(self, client, tmp_path: Path, monkeypatch) -> None:
        compose_dir = tmp_path / "compose"
        compose_dir.mkdir()
        (compose_dir / "docker-compose.yml").write_text("services: {}\n", encoding="utf-8")
        monkeypatch.setattr(
        ...
        tag = client.get("/__ui__/api/compose/docker-compose.yml").headers["etag"]
        for marker in ("a", "b", "c"):
            response = client.put(
                "/__ui__/api/compose/docker-compose.yml",
                json={"content": f"services: {{{marker}: {{}}}}\n"},
                headers={"If-Match": tag},
            )
            assert response.status_code == 200, (marker, response.text)
            tag = response.headers["etag"]
            assert tag == etag_for_file(compose_dir / "docker-compose.yml")
        assert "c" in (compose_dir / "docker-compose.yml").read_text(encoding="utf-8")

    def test_action_map_can_be_saved_twice_in_a_row(self, client) -> None:
        tag = client.get("/__ui__/api/rules_action_map").headers["etag"]
        for action in ("review", "sanitize", "block"):
            response = client.patch(
                "/__ui__/api/rules_action_map",
                json={"sanitizer": {"dangerous_command": action}},
                headers={"If-Match": tag},
            )
            assert response.status_code == 200, (action, response.text)
            tag = response.headers["etag"]
        assert (
            client.get("/__ui__/api/rules_action_map").json()["action_map"]["sanitizer"][
                "dangerous_command"
            ]
            == "block"
        )

    def test_config_can_be_saved_twice_in_a_row(self, client) -> None:
        tag = client.get("/__ui__/api/config").headers["etag"]
        for level in ("debug", "warning", "error"):
            response = client.post(
                "/__ui__/api/config",
                json={"values": {"log_level": level}},
                headers={"If-Match": tag},
            )
            assert response.status_code == 200, (level, response.text)
            tag = response.headers["etag"]
        assert "AEGIS_LOG_LEVEL=error" in client.env_path.read_text(encoding="utf-8")
