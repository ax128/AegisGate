...
async def _async_fake_resolve_upstream(headers):
...
def _install_common_passthrough_mocks(monkeypatch: pytest.MonkeyPatch) -> list[str]:
    ...
    monkeypatch.setattr(
        openai_router, "_resolve_upstream_base", _async_fake_resolve_upstream
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        openai_router, "_run_response_pipeline", _raise_unexpected_async
    )
    monkeypatch.setattr(
        openai_router, "_apply_semantic_review", _raise_unexpected_async
    )
...
def test_responses_stream_debug_log_filter() -> None:
    ...
    assert (
        openai_router._should_log_responses_stream_event("response.completed") is False
    )
    assert (
        openai_router._should_log_responses_stream_event("response.output_text.delta")
        is False
    )
...
def test_filter_done_debug_log_only_when_filter_hits() -> None:
    assert (
        pipeline_module._should_log_filter_done(
            phase="request", is_stream=False, report={"hit": False}
        )
        is False
    )
    assert (
        pipeline_module._should_log_filter_done(
            phase="request", is_stream=False, report={"hit": True}
        )
        is True
    )
    assert (
        pipeline_module._should_log_filter_done(
            phase="response", is_stream=True, report={"hit": True}
        )
        is False
    )
...
@pytest.mark.asyncio
async def test_responses_endpoint_redirects_chat_json_back_to_responses_shape(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_inline_payload_transform(monkeypatch)
    payload = {
...
@pytest.mark.asyncio
async def test_responses_endpoint_redirects_chat_stream_back_to_responses_events(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_inline_payload_transform(monkeypatch)
    payload = {
    ...
    assert (
        b'"type": "response.output_text.delta"' in body
        or b'"type":"response.output_text.delta"' in body
    )
...
async def test_responses_endpoint_redirects_chat_stream_tool_calls_back_to_responses_events(
    ...
    body = await _collect_stream_body(result)
    assert (
        b'"type":"response.output_item.added"' in body
        or b'"type": "response.output_item.added"' in body
    )
    assert (
        b'"type":"response.output_item.done"' in body
        or b'"type": "response.output_item.done"' in body
    )
    assert (
        b'"type":"response.completed"' in body
        or b'"type": "response.completed"' in body
    )
...
async def test_passthrough_json_endpoints_skip_all_filters(
    ...
    async def fake_forward_json(
        url: str, forwarded_payload: dict[str, object], headers: dict[str, str]
    ):
...
@pytest.mark.asyncio
async def test_chat_passthrough_preserves_structured_content(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "model": "gpt-5.4",
...
@pytest.mark.asyncio
async def test_messages_passthrough_preserves_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
...
@pytest.mark.asyncio
async def test_messages_compat_openai_chat_delegates_to_responses_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "model": "claude-sonnet-4.5",
...
@pytest.mark.asyncio
async def test_messages_compat_openai_chat_returns_messages_tool_use_blocks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "model": "claude-sonnet-4.5",
...
@pytest.mark.asyncio
async def test_messages_compat_openai_chat_stream_preserves_tool_use_blocks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "model": "claude-sonnet-4.5",
        "messages": [{"role": "user", "content": "run tool"}],
        "max_tokens": 128,
        "request_id": "messages-compat-tool-stream",
        "session_id": "messages-compat-tool-stream",
        "stream": True,
    }

    async def fake_execute_responses_stream_once(**kwargs):
        async def responses_generator() -> AsyncGenerator[bytes, None]:
            yield (
                b'data: {"type":"response.output_item.done","item":{"type":"function_call","call_id":"call_1","name":"lookup_profile","arguments":"{\\"user_id\\": 7}"}}\n\n'
            )
            yield (
                b'data: {"type":"response.completed","response":{"id":"resp-tool-1","status":"completed","output":[{"type":"function_call","call_id":"call_1","name":"lookup_profile","arguments":"{\\"user_id\\": 7}"}]}}\n\n'
            )
            yield b"data: [DONE]\n\n"
    ...
    monkeypatch.setattr(
        ...
        fake_execute_responses_stream_once,
    )
    ...
    assert b'"type": "tool_use"' in body or b'"type":"tool_use"' in body
    assert b'"name": "lookup_profile"' in body or b'"name":"lookup_profile"' in body
    assert b'"stop_reason": "tool_use"' in body or b'"stop_reason":"tool_use"' in body


@pytest.mark.asyncio
async def test_responses_stream_preserves_event_and_data_order(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    audit_calls: list[str] = []
    _install_inline_payload_transform(monkeypatch)
    ...
    monkeypatch.setattr(
        openai_router, "_run_request_pipeline", _identity_request_pipeline
    )
    ...
    assert body.index(b"event: response.output_text.delta") < body.index(
        b"event: response.output_text.done"
    )
    assert body.index(b"event: response.output_text.done") < body.index(
        b"event: response.completed"
    )
    assert body.endswith(b"data: [DONE]\n\n")
