...
宿主机 `./config` 会挂载为容器内策略目录。**首次启动时若目录为空**，应用会在启动时自动从内置默认生成 `.env` 与策略 YAML 到该目录，无需手动复制。

**让 ./logs 和 ./config 可写**（容器内以 uid 10001 运行，否则数据库、审计、token 无法落盘）：首次启动前在项目根目录执行：

```bash
chown -R 10001:10001 logs config
```

再执行 4.3 启动。之后 `./logs` 下会出现 `aegisgate.db`、`audit.jsonl`、`gw_tokens.json` 等。
